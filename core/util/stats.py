...
def _sample_tangent_unit(mu):
    #mat = np.matrix(mu)
    #if mat.shape[1]>mat.shape[0]:
    #    mat = mat.T
    #U,_,_ = la.svd(mat)
    #nu = np.matrix(np.random.randn(mat.shape[0])).T
    nu = np.random.randn( len(mu),1 )
    U,_,_ =  scipy.linalg.svd(mu.reshape((len(mu),1)))
    x = np.dot(U[:,1:],nu[1:,:])
    return (x/ scipy.linalg.norm(x)).reshape( (len(x),) )
...
def sample_vMF_lognormal(mu_0, C_0, m, sigma,size=1):
        kappa = np.random.lognormal(m,sigma,size)
        mu = sample_vMF(mu_0, C_0,size)
        return mu, kappa
